...
<div id="wrapper">
	...
	<?php
		...
		echo "<b>Appointment booked unsuccessfully.</b><br>
		<i>Go to payment tab to complete the payment by the next hour. </i> <br><br>";
		echo 
		...
		"<a href='payment.php'><button id='viewBtn'>Proceed to Payment Tab</button></a>" .
		...
		$query = "insert into appointments values
				(NULL,'".$location."','".$doctor."', '".$date."', '".$time."', '".$timeCompleted."' ,false, true)";
		$result = $db->query($query);
		if ($result) {
			echo "<p><br>Please pay within the next hour by: <b><u>" . $timePay2 . 
			"</u></b><br>Or else cancel the preappointment slot to free up the slot for other patients.</p>";
			} else {
